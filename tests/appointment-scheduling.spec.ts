...
test('Appointment Scheduling – Mandatory Fields', async ({ page }) => {
  ...
  await calendarModal.locator('[role="gridcell"]').filter({ hasText: '15' }).click();
  ...
  // Try to select a time slot using predefined selectors
  ...
  await expect(page.getByText('Rutuja Dumbre').first()).toBeVisible({ timeout: 10000 });
